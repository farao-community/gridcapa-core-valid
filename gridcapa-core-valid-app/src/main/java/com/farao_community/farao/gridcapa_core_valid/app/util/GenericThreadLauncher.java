...
package com.farao_community.farao.gridcapa_core_valid.app.util;
...
public class GenericThreadLauncher<T, U> extends Thread {
    ...
    private static Method getMethodAnnotatedWith(final Class<?> type) {
        ...
        if (methods.isEmpty()) {
            throw new CoreValidInternalException("the class " + type.getCanonicalName() + " does not have his running method annotated with @Threadable");
        } else if (methods.size() > 1) {
            throw new CoreValidInternalException("the class " + type.getCanonicalName() + " must have only one method annotated with @Threadable");
        } else {
