...
class NetPositionsHandlerTest {
    ...
    void shiftNetPositionTestWithAlegroShift() {
        ...
        assertEquals(600.0, network.getBoundaryLineStream().filter(dl -> dl.getPairingKey().equals("XLI_OB1B")).findAny().get().getP0(), 0.01);
        assertEquals(-600.0, network.getBoundaryLineStream().filter(dl -> dl.getPairingKey().equals("XLI_OB1A")).findAny().get().getP0(), 0.01);
    }
    ...
    void shiftNetPositionTestWithAlegroRefProg() {
        ...
        assertEquals(-234.0, network.getBoundaryLineStream().filter(dl -> dl.getPairingKey().equals("XLI_OB1B")).findAny().get().getP0(), 0.01);
        assertEquals(234.0, network.getBoundaryLineStream().filter(dl -> dl.getPairingKey().equals("XLI_OB1A")).findAny().get().getP0(), 0.01);
    }
}
